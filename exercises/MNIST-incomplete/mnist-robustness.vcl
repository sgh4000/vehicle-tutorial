...
-- Creating a strong advises for the strong classifier robustness

strongAdvises : Image -> Label -> Bool
strongAdvises x i =
  forall j . j != i => classifier x ! i >= classifier x ! j + eta
...
-- Want to ensure that for every small perturbation (within ε and still producing a valid image), the classifier’s output on that perturbed image still satisfies strongAdvises.
